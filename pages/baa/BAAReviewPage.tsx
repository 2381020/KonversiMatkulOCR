...
import { BAADetailPage } from './BAADetailPage';

interface BAAReviewPageProps {
  user: User;
}

export const BAAReviewPage: React.FC<BAAReviewPageProps> = ({ user }) => {
  ...
  const loadRequests = async () => {
    try {
      const data = await conversionService.getAllPendingRequests('pending_baa');
      setRequests(data);
    } catch (error) {
  ...
  if (selectedRequest) {
    return <BAADetailPage request={selectedRequest} onBack={handleBack} user={user} />;
  }

  return (
    <div>
      <h1 className="text-3xl font-bold text-slate-800 mb-6">Review Konversi - BAA</h1>
                    ...
                    <tr key={request.id} className="bg-white border-b hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-center">
                        <button
                          ...
                          className="font-medium text-blue-600 hover:text-blue-800 px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors"
                        >
                          Review
                        </button>
                      </td>
